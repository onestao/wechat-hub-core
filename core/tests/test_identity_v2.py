...
class IdentityV2CrossModuleIntegrationTests(unittest.TestCase):
    ...
    def test_e2e_real_runtime_registry_to_core_alignment(self):
        ...
        repo_root = next(
            (p for p in Path(__file__).resolve().parents if (p / ".worktrees" / "runtime-release").is_dir() or (p / "work" / "runtime").is_dir()),
            None,
        )
        if repo_root and (repo_root / ".worktrees" / "runtime-release" / "root" / "scripts" / "wechat").is_dir():
            runtime_scripts = repo_root / ".worktrees" / "runtime-release" / "root" / "scripts" / "wechat"
        elif repo_root and (repo_root / "work" / "runtime" / "root" / "scripts" / "wechat").is_dir():
            runtime_scripts = repo_root / "work" / "runtime" / "root" / "scripts" / "wechat"
        else:
